(function(){
  ...
  const activityNames={lobby:'Welcome screen (QR Code)',lens:'Live Nigeria map',passport:'Passport trivia',decode:'Decode the State'};
  ...
  function applyAdminStatus(status){
    ...
    const currentClue=status.session?.current_clue||status.session?.currentClue||1;
    select.innerHTML='<option value="">Choose a ready question</option>';
    status.questions.filter(q=>(!q.activity||q.activity===activity)&&(state==='preparing'?true:q.id!==currentId)).forEach(q=>select.add(new Option(`${q.category}: ${q.question}`,q.id)));
  ...
  function normaliseQuestion(q){const options=(q.question_options||q.options||[]).map((o,i)=>typeof o==='string'?{option_index:i,label:o}:o).sort((a,b)=>a.option_index-b.option_index);return {...q,activity:q.activity||q.quiz_rounds?.quiz_games?.activity,options,correctOption:q.correct_option??q.correctOption??0,durationSeconds:q.duration_seconds??q.durationSeconds??20,reviewStatus:q.review_status??q.reviewStatus??'requires_fact_check',media:q.media||null,fallback:q.image_fallback||q.fallback||null}}
  function renderQuestions(){const query=$('#question-search').value.trim().toLowerCase(),status=$('#question-status-filter').value;const filtered=questions.filter(q=>(status==='all'||q.reviewStatus===status)&&`${q.category} ${q.question}`.toLowerCase().includes(query));$('#question-count').textContent=`${filtered.length} of ${questions.length}`;$('#question-list').innerHTML=filtered.map(q=>`<article class="question-card"><div class="question-card-meta"><span>${escape(q.activity==='decode'?'Decode · '+q.category:'Passport · '+q.category)}${q.media?.src?` · 📷 ${q.media.timing==='reveal'?'Reveal photo':'Photo'}`:''}</span><span class="review-badge ${escape(q.reviewStatus)}">${statusLabels[q.reviewStatus]||escape(q.reviewStatus)}</span></div><h2>${escape(q.question)}</h2><p><strong>Correct answer:</strong> ${escape(q.options[q.correctOption]?.label||'Not set')} <span class="question-time">${q.durationSeconds} seconds</span></p><button class="button secondary edit-question" data-id="${q.id}">Edit question</button></article>`).join('')||'<div class="panel"><h2>No matching questions</h2><p>Try a different search or review filter.</p></div>';$$('.edit-question').forEach(button=>button.onclick=()=>openEditor(button.dataset.id))}
  function openEditor(id){const q=questions.find(x=>x.id===id);if(!q)return;$('#editor-id').value=q.id;$('#editor-category').textContent=q.category;$('#editor-question').value=q.question;q.options.forEach((option,i)=>{$(`#editor-option-${i}`).value=option.label});const correct=$(`[name="correct-option"][value="${q.correctOption}"]`);if(correct)correct.checked=true;$('#editor-duration').value=q.durationSeconds;$('#editor-status').value=q.reviewStatus;$('#editor-explanation').value=q.explanation||'';$('#editor-source').value=q.source||'';const m=q.media;$('#editor-media-src').value=m?.src||'';$('#editor-media-timing').value=m?.timing||'';$('#editor-media-alt').value=m?.alt||'';$('#editor-media-caption').value=m?.caption||'';$('#editor-media-author').value=m?.author||'';$('#editor-media-license').value=m?.license||'';$('#editor-media-fallback').value=q.fallback||q.image_fallback||'';show($('#editor-message'),'');$('#question-editor').showModal()}
  async function saveQuestion(event){event.preventDefault();const button=$('#save-question'),id=$('#editor-id').value;const mediaSrc=$('#editor-media-src').value.trim(),mediaTiming=$('#editor-media-timing').value;const media=mediaSrc&&mediaTiming?{src:mediaSrc,timing:mediaTiming,alt:$('#editor-media-alt').value.trim(),caption:$('#editor-media-caption').value.trim(),author:$('#editor-media-author').value.trim(),license:$('#editor-media-license').value.trim()}:null;const fallback=$('#editor-media-fallback').value.trim()||null;const payload={id,question:$('#editor-question').value,options:[0,1,2,3].map(i=>$(`#editor-option-${i}`).value),correctOption:Number($('[name="correct-option"]:checked')?.value),durationSeconds:Number($('#editor-duration').value),reviewStatus:$('#editor-status').value,explanation:$('#editor-explanation').value,source:$('#editor-source').value,media,fallback};try{button.disabled=true;show($('#editor-message'),'Saving…');const result=await api().request('/admin/question',{method:'PATCH',admin:true,body:payload});const saved=normaliseQuestion(result.question);questions=questions.map(q=>q.id===id?saved:q);renderQuestions();show($('#content-message'),'Question saved. The event controls now use the updated version.');$('#question-editor').close()}catch(err){show($('#editor-message'),err.message,true)}finally{button.disabled=false}}
