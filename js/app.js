(function(){
  ...
  async function initWelcome(){const form=$('#join-form');form?.addEventListener('submit',async e=>{e.preventDefault();const result=core().validateAlias($('#alias').value),msg=$('#join-message');if(!result.valid)return message(msg,result.error,true);try{$('#join-button').disabled=true;await api().join(result.value);location.href=new URLSearchParams(location.search).get('next')||'/activities'}catch(err){message(msg,err.message,true)}finally{$('#join-button').disabled=false}});if(api().getProfile()){$('#existing').classList.remove('hidden');$('#existing-name').textContent=api().getProfile().alias}initWelcomeMap()}
