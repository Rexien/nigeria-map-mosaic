  ...
  async function request(path,{method='GET',body,admin=false,idempotencyKey,timeout=8000}={}){
    if(path==='/answers'&&method==='POST'&&gatewayAnswerUrl){
      ...
      try{
        ...
        if(response.status<500)throw Object.assign(new Error(apiErrorMessage(data,'Answer was rejected')),{status:response.status,code:data.code||data?.error?.code,noFallback:true});
