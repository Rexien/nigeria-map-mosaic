...
test('personal score uses one authenticated read and retains the latest revealed snapshot', async () => {
  ...
  global.fetch = async url => {
    ...
    assert.match(String(url), /participant_score_snapshots\.order=created_at\.desc/);
    return new Response(JSON.stringify([{
      id: 'player-1', alias: 'Player', registered_at: '2026-09-01T00:00:00Z',
      is_spectator: false,
      participant_score_snapshots: [{ rank: 3, scores: { day1: 800, day2: 0, combined: 800, decode: 0 }, stamps: ['first'] }]
    }]), { status: 200 });
  };
  try {
