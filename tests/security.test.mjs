...
test('question edits are authenticated, validated, and audited',()=>{assert.match(api,/route\s*===\s*'admin\/question'/);assert.match(api,/update_question/);assert.match(api,/options\.length\s*!==\s*4/);assert.match(api,/This question is being shown now/)});
