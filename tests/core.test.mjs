...
test('the host state machine blocks invalid automatic progression',()=>{assert.equal(c.canTransition('open','revealed'),false);assert.equal(c.canTransition('open','locked'),true);assert.equal(c.canTransition('locked','revealed'),true);assert.equal(c.canTransition('revealed','lobby'),true);assert.equal(c.canTransition('leaderboard','lobby'),true)});
