...
const normalizeSecret=value=>typeof value==='string'?value.trim():value;
const DEFAULT_SECRET = normalizeSecret(process.env.CREDENTIAL_SECRET_ACTIVE) || (process.env.NODE_ENV==='production' ? null : 'dev-credential-secret-2026');
const PREVIOUS_SECRET = normalizeSecret(process.env.CREDENTIAL_SECRET_PREVIOUS) || null;
...
export function verifyParticipantCredential(token, secret = DEFAULT_SECRET, previousSecret = PREVIOUS_SECRET) {
  ...
  const candidates = new Set([normalizeSecret(secret),normalizeSecret(previousSecret)].filter(Boolean));

  let verified = false;
  for (const candidate of candidates) {
    const expectedSig = computeHmac(payloadB64, candidate);
