...
export function computeScoresAndRanks({ participants = [], answers = [], questions = [] }) {
  ...
  // Bucket answers by participantId
  ...
  for (const a of answers) {
    ...
    const points = a.points != null
      ? Number(a.points)
      : (is_correct ? (activity==='decode'?Math.max(1,4-clueNumber):Number(qInfo?.points ?? 1000)) : 0);
  ...
  for (const [pId, pData] of participantDetails) {
    ...
    const rank = isSpectator ? null : (passportRanks.get(pId) || 1);
    const decodeRank = isSpectator ? null : (decodeRanks.get(pId) || 1);
    const record = {
      participantId: pId,
      isSpectator,
      rank,
  ...
  const leaderboardSnapshots = [
    {
      ...
      activity: 'passport',
      snapshotVersion,
      leaders: computed.passportLeaders,
      createdAt
    },
    {
      sessionId,
      activity: 'decode',
      snapshotVersion,
      leaders: computed.decodeLeaders,
      createdAt
    }
  ];
