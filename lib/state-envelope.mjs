...
import { publicQuestionMedia } from './question-media.mjs';
...
export function sanitizePublicQuestion(question, state) {
  ...
  if (question.cluesSoFar) safe.cluesSoFar = question.cluesSoFar;
  if (question.clueMediaSoFar) {
    safe.clueMediaSoFar = question.clueMediaSoFar.map(m => {
      if (!m) return null;
      return {
        src: m.src,
        alt: m.alt || '',
