...
8. **SSE disconnect cleanup depends on proxy behavior.** The checked-in gateway Caddy template previously set `flush_interval -1`; Caddy documents that this keeps the upstream request running after the downstream client disconnects. Event-stream responses are already flushed immediately. Remove the negative interval and verify an aborted client disappears from the gateway's active-client count; the live host's Caddy config still needs this change and verification.
9. **Shared-event preflight must be a hard gate.** The manual load workflow must require an explicit quiet-window confirmation and stop before participant creation unless the gateway has zero listeners, an empty queue, a configured durable sink, a non-open question state, and matching health/state responses. Operator-only checks should remain visible as warnings rather than making the automated gate permanently false.
...
| Vercel | Function errors, duration/timeouts, invocation/concurrency evidence where available, usage delta and logs linked by request ID and test time. `/api/me` logs include `dependencyTiming.durationMs` for the single Supabase REST snapshot read, alongside total handler duration, so database-path time can be compared with end-to-end client latency. Separate cold startup from warmed repetitions. |
...
| Duplicate submission prevention | **genuine gap requiring a change** across the complete gateway/fallback path | The old RPC/schema mismatch described in the prior draft is fixed in current source. Exact option/session/question/idempotency are retried, but a gateway-accepted answer whose acknowledgement is lost can race the Vercel fallback into Supabase first; generated answer ID and server-measured response time can then differ from the locally accepted gateway record because the gateway sink ignores conflicts. This is not yet proven safe end-to-end. |
...
Normal gateway path: SQLite has unique participant/question and participant/idempotency indexes, transaction commits, and duplicate acknowledgements await the original commit outcome. `server/api.mjs::answer` now validates UUID session/question/idempotency fields, confirms the requested session and active question, and calls the same `submit_raw_quiz_answer(text,uuid,uuid,smallint,uuid)` signature defined by the tracked migration. That RPC returns the prior row on duplicate and binds the first stored option. The ancillary counter is `count_quiz_answers`, not the old undocumented `increment_live_response_count`. Local coverage includes `tests/queue-durability.test.mjs`, `tests/fallback-contract.test.mjs`, and `tests/load-harness.test.mjs`; none proves the hosted gateway-to-Supabase lost-ack race.

**Remaining cross-route durability gap:** the browser persists and retries the same session, question, option, and UUID idempotency key. But the gateway acknowledges after local SQLite durability, before Supabase flush. If that acknowledgement is lost and the browser falls back to Vercel before the gateway row flushes, the RPC may insert a separate Supabase row. Later the gateway sink uses `resolution=ignore-duplicates` on participant/question and marks its SQLite row flushed without comparing answer ID or `response_ms`. The selected option remains the same under the current browser retry contract, but the durable row/ID and response-time tie-break can differ from the locally accepted gateway record. This deserves a narrow contract/design fix or an explicit accepted semantic before capacity sign-off; no shared score counter or PgBouncer change addresses it.

**Test DUP-1 (contract smoke before scale):** confirm the hosted RPC/migration signature and constraints, then force gateway transport failure for one player and exercise the real browser fallback. Assert the exact request identity is retained, stale/missing/mismatched IDs are rejected, the RPC result corresponds to one durable answer, and the duplicate response does not alter the original option. This local contract test currently passes; repeat against the authorized Preview and verify deployed SQL before scale.

**Test DUP-2 (parallel/recovery):** burst 10–20% same-payload retries; include same-key changed-option and different-key same-question attempts, local commit failure, gateway acknowledgement loss after local commit, and fallback racing a delayed durable flush. Reconcile answer ID, option, session/question, and response time—not only row counts. Require exactly one logical answer/award; no success without local or durable confirmation; the durable row must match the defined canonical receipt. Include deadline/reveal and next-question boundaries. The lost-ack/fallback case is currently unverified and is a pre-scale sign-off gate.

### Sign-off boundary and coordination

The five checks do not mandate a stack change. Pooling, push transport, caching and snapshot scoring are the right architectural choices, subject to hosted capacity/fanout/race testing. Duplicate RPC contract and exact browser payload retry are locally covered, but cross-route lost-ack durability equivalence remains a pre-scale gate. Current source/unit/browser checks do not establish deployed capacity. Before the large rehearsal: (1) resolve or explicitly accept the fallback answer-ID/response-time semantics, (2) get Vercel Preview access or configure the authorized GitHub bypass path, (3) verify Preview authority and shared Oracle/Supabase identities plus queue-empty/monitoring preflight, (4) run a tiny deployed fallback smoke, then staged k6/Node SSE and burst tiers, (5) reconcile all accepted receipts against durable rows/snapshots, and (6) only then consider any production smoke separately. Shared rehearsal still needs monitoring and must not delete participant data without explicit authorization.
